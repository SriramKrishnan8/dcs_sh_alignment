...
not_generated, sh_file_missing, missed_chunks, dcs_file_missing = sys.argv
...
#4 - stem
#5 - stem_sense
#6 - inflectional_morph
#7 - base_stem
#8 - base_stem_sense
#9 - derivational_morph
#10 - pre_verb
...
def check_for_avyaya(sent_id, stem, word, inf_morph, der_morph):
    """ Checks the list of all avyayas whose morph analyses is not 
        present in the DCS. This list is obtained prior to the 
        alignment process.
    """
    
#    print("\t".join((sent_id, stem, word, inf_morph, der_morph)))
    condition = (
        (inf_morph in avyayas) and
        (((sent_id, stem) in avyayas_list) or
        ((sent_id, word) in avyayas_list))
    )
    
    return condition


def align_morph(sent_id, stem, word, dcs_morph, inf_morph, der_morph):
    """ Conditions for mapping DCS and SH morph analyses """
    
    if dcs_morph == "":
        return check_for_avyaya(sent_id, stem, word, inf_morph, der_morph)
...
def compare_dcs_sh(sent_id, dcs_stem, dcs_morph, sh_nodes):
...
#        print(node)
        ...
        morphs_condition = align_morph(sent_id, dcs_stem, word, dcs_morph, 
                                       inf_morph, der_morph)
        
#        print("Stem: " + str(stems_conditions))
#        print("Morph: " + str(morphs_condition))
...
def align(data, all_nodes):
    ...
    for i in range(len(stems)):
        ...
        for j in range(len(stems[i])):
            segment_nodes = compare_dcs_sh(str(sent_id), stems[i][j], 
                                           morphs[i][j], all_nodes)
...
#    print(segment_nodes)
    ...
    for i in range(len(position)):
        ...
        for j in range(len(position[i])):
            ...
            sh_word = current_segment_nodes[1]
            word = an.to_anunasika(sh_word)
            phase = current_segment_nodes[2]
            phase_color = current_segment_nodes[3]
            sh_stem = current_segment_nodes[4]
            sh_stem_sense = current_segment_nodes[5]
            stem = an.to_anunasika(sh_stem)
            sh_morph = current_segment_nodes[6]
            sh_base = current_segment_nodes[7]
            sh_base_sense = current_segment_nodes[8]
            base = an.to_anunasika(sh_base)
            sh_base_morph = current_segment_nodes[9]
            pre_verb = current_segment_nodes[10]
            ...
            if compound_component(phase) or iic_state:
                comp_entries.append((sent_id, str(i), str(j), dcs_stem,
                                    dcs_stem_id, dcs_morph, dcs_word, sh_word,
                                    phase, sh_stem, sh_stem_sense, sh_base,
                                    sh_base_sense, sh_morph, sh_base_morph,
                                    word, stem, base))
            else:
                pada_entries.append((sent_id, str(i), str(j), dcs_stem, 
                                    dcs_stem_id, dcs_morph, dcs_word, sh_word,
                                    phase, sh_stem, sh_stem_sense, sh_base,
                                    sh_base_sense, sh_morph, sh_base_morph,
                                    word, stem, base))
...
# To record all the sentences whose dcs file is missing
dcs_file_missing_file = open(dcs_file_missing, "a+")
...
#    print(cm.list_to_string_for_file(all_nodes))
    ...
    all_nodes = es.discard_chunk_info(all_nodes)
...
#    print(cm.list_to_string_for_file(all_nodes))
    ...
    if not os.path.isfile(dcs_path):
        dcs_file_missing_file.write(line + "\n")
        continue
